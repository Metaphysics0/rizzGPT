<script lang="ts">
  ...
  import type { ActionData } from "../../../routes/sign-in/$types";
  ...
  >
    ...
    <div class="space-y-4">
      <button
        type="submit"
        disabled={isLoading}
        class="cursor-pointer group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-lg text-white bg-pink-600 hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {#if isLoading}
          <svg
